...
class Server:
    ...
    def __init__(self):
        ...
        dp.add_handler(MessageHandler([Filters.text], self.command_check_resps))
        dp.add_handler(CommandHandler("start", self.command_start))
        dp.add_handler(CommandHandler("roll", self.command_roll, pass_args=True))
    ...
    def command_start(self, bot, update):
        ...
        if chat_id not in self.SESSIONS.keys():
            ...
            self.SESSIONS[chat_id] = Session(chat_id, self.config_instance, self.logger)
    ...
    def command_roll(self, bot, update, args=''):
        ...
        self.SESSIONS[chat_id].messenger.send_msg(chat_id, _("searching_movies"))
    ...
    def command_action(self, bot, update):
        ...
        try:
            ...
            self.SESSIONS[update.message.chat_id].messenger.send(update, player.name + " entrou na partida!")
        except ValueError as e:
            self.SESSIONS[update.message.chat_id].messenger.send(update, update.message.from_user.first_name + e.args[0])

    def command_repeat(self, bot, update):
        movie_img = self.SESSIONS[update.message.chat_id].quiz.get_question()
        self.SESSIONS[update.message.chat_id].messenger.send(update, "=========REPETINDO=========")
    ...
    def command_stop(self, bot, update):
        try:
            ...
            self.SESSIONS[update.message.chat_id].messenger.send(update, "Encerrando a partida...")
        except ValueError as e:
            self.SESSIONS[update.message.chat_id].messenger.send(update, "Não foi possível encerrar a partida: %s" % e)
